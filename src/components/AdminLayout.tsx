...
const AdminLayout = ({ children, title }: AdminLayoutProps) => {
  ...
  const menuItems = [
    ...
    { icon: FileText, label: 'จัดการใบสมัคร', href: '/dashboard/applications' },
  ...
  const getInitials = (name: string) => {
    ...
    if (parts.length >= 2) {
      return parts[0].replace('อาจารย์', '').charAt(0) + parts[1].charAt(0);
    }
      ...
      >
        ...
        <div className="p-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            {(sidebarOpen || isMobile) && <span className="font-bold text-lg">P-BAC อาจารย์</span>}
          </div>
          {isMobile && sidebarOpen && (
            <button onClick={() => setSidebarOpen(false)} className="p-1 hover:bg-primary-foreground/10 rounded">
              <X size={20} />
            </button>
          )}
        </div>
        ...
        <div className="p-3 border-t border-primary-foreground/10 space-y-1">
          <Link
            to="/"
            className="flex items-center gap-3 px-4 py-3 rounded-lg text-primary-foreground/80 hover:bg-primary-foreground/10 transition-all w-full"
          >
